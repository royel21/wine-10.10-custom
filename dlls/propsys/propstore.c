...
HRESULT WINAPI PSCreatePropertyStoreFromObject(IUnknown* obj, DWORD access, REFIID riid, void** ret)
{
    ...
    /* If the object already supports IPropertyStore, just return it */
    if (IsEqualIID(riid, &IID_IPropertyStore) && SUCCEEDED(hr = IUnknown_QueryInterface(obj, riid, ret)))
        return hr;

    /* Otherwise, create a new PropertyStore instance */
    if (IsEqualIID(riid, &IID_IPropertyStore))
    {
        hr = PropertyStore_CreateInstance(NULL, riid, ret);
        return hr;
    }
